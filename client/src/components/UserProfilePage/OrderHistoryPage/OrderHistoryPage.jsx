...
import { OrderDetailsPage } from "../index.js";
...
const OrderHistoryPage = () => {
  ...
  const fetchBookings = async () => {
    try {
      let AllBookingResponse = await axios.get(
        `${import.meta.env.VITE_SERVER_URL}/eventify_server/bookingMaster/`
      );
      let confirmedResponse = await axios.get(
      ...
      const vendorResponse = await axios.get(
        ...
        }/eventify_server/serviceProviderMaster/`
      );

      const hallsData = hallsResponse.data;
      const customersData = customersResponse.data;
      const vendorsData = vendorResponse.data;
      const bookingsWithHallAndCustomerInfo = AllBookingResponse.data.map(
        (booking) => {
          const hallData = hallsData.find(
            (hall) => hall._id === booking.hallId
          );
          const customerData = customersData.find(
            (customer) => customer._id === booking.customerId
          );
          const serviceProvider = vendorsData.find(
            (vendor) => vendor._id === booking.hallUserId
          );
      ...
      const confirmedBookingsWithHallAndCustomerInfo =
        confirmedResponse.data.map((booking) => {
          const hallData = hallsData.find(
            (hall) => hall._id === booking.hallId
          );
          const customerData = customersData.find(
      ...
      if (userType === "ADMIN") {
        ...
        const vendorPendingBookingsData =
          bookingsWithHallAndCustomerInfo.filter(
            (booking) =>
              booking.hallUserId === userId &&
              (booking.bookingStatus == "PENDING" ||
                booking.bookingStatus === "ONHOLD")
          );
        const vendorCanceledBookingsData =
          bookingsWithHallAndCustomerInfo.filter(
            (booking) =>
              booking.hallUserId === userId &&
              booking.bookingStatus === "REJECTED"
          );
        const vendorBookings = vendorPendingBookingsData.concat(
          vendorCanceledBookingsData
        );
        setBookings(vendorBookings);

        const vendorConfirmedBookings =
          confirmedBookingsWithHallAndCustomerInfo.filter(
            (booking) => booking.hallUserId === userId
          );
        setConfirmedBookings(vendorConfirmedBookings);
      } else if (userType === "CUSTOMER") {
        const customerPendingBookingsData =
          bookingsWithHallAndCustomerInfo.filter(
            (booking) =>
        ...
        const customerCanceledBookingsData =
          bookingsWithHallAndCustomerInfo.filter(
            (booking) =>
              booking.customerId === userId &&
              booking.bookingStatus === "REJECTED"
          );
        const customerBookings = customerPendingBookingsData.concat(
          customerCanceledBookingsData
        );
        setBookings(customerBookings);

        const customerConfirmedBookings =
          confirmedBookingsWithHallAndCustomerInfo.filter(
            ...
            {bookings.length === 0 && confirmedBookings.length === 0 ? (
              ...
              <div className="order-list">
                {[...bookings, ...confirmedBookings].map((order) => {
                  const isConfirmed = confirmedBookings.some(
                    (confirmedBooking) => confirmedBooking._id === order._id
                  );
